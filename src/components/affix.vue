...
<script>
	export default {
		...
		computed: {
			...
			classList(){
				return {
					'affix': this.classState.affix,
					'affix-top': this.classState.affixTop,
					'affix-bottom': this.classState.affixBottom,
					'vue-affix': this.classState.vueAffix,
				}
			}
		},
		...
		watch: {
			...
			enabled(val){
				this.classState.vueAffix = false;
				this.affixInitialTop = null;
		...
		methods: {
			...
			setScrollAffixScrolling() {
				...
				this.styles.top = `${(Math.floor(this.affixRect.top) + this.topOfScreen) - this.affixInitialTop}px`;
				this.styles.bottom = '';
				this.removeClasses();
				this.emitEvent();
			},
			...
			setScrollAffixUp() {
				...
				if (this.currentScrollAffix !== this.lastScrollAffixState) {
					this.styles.top = `${this.topPadding + this.offset.top}px`;
					this.styles.bottom = '';
					this.removeClasses();
					this.emitEvent();
					this.classState.affix = true;
				}
			},
			...
			setScrollAffixDown() {
				...
				if (this.currentScrollAffix !== this.lastScrollAffixState) {
					this.styles.bottom = `${this.offset.bottom}px`;
					this.styles.top = '';
					this.removeClasses();
					this.emitEvent();
					this.classState.affix = true;
			...
			setScrollAffixBottom() {
				this.currentScrollAffix = 'scrollaffix-bottom';
				this.styles.top = `${this.relativeElmBottomPos - this.affixInitialTop - this.affixHeight}px`;
				this.styles.bottom = '';
				this.removeClasses();
				this.emitEvent();
			},
			...
			setAffixTop() {
				...
				if (this.currentState !== this.lastState) {
					this.emitEvent();
					this.removeClasses();
					this.classState.affix = false;
					this.classState.affixTop = true;
					this.styles.top = '';
				}
			},
			...
			setAffixBottom() {
				...
				this.styles.top = `${this.relativeElement.offsetHeight - this.affixHeight
				- this.offset.bottom - this.topPadding}px`;

				if (this.currentState !== this.lastState) {
					this.emitEvent();
					this.removeClasses();
					this.classState.affixBottom = true;
				}
			},
			 ...
			 */
			removeClasses() {
				this.classState.affixTop = false;
				this.classState.affix = false;
				this.classState.affixBottom = false;
			},
			...
			changed(){
				clearTimeout(this.positionObserverTimer);
				this.positionObserverTimer = setTimeout(()=>{
					this.updateData();

					if (this.enabled && this.affixHeight < window.innerHeight - (this.topPadding + this.offset.top + this.offset.bottom)){
						this.setScrollAffixUp();
					}
				}, 100)
			},
			...
			startObserve(){
				this.positionObserver = new MutationObserver(this.changed);

				this.positionObserver.observe(this.$el, {
					childList: true,
					attributes: true,
					subtree: true
				});
